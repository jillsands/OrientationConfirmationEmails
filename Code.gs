...
function openReadMe() {
  var htmlOutput =
    HtmlService.createHtmlOutputFromFile('openURL').setHeight(40);
...
function parseData(submission) {
  ...
  if (
    submission['Program Type'] == 'FO' ||
    submission['Program Type'] == 'TO'
  ) {
    submission.charge = 95;
  } else if (
    submission['Program Type'] == 'F1' ||
    submission['Program Type'] == 'T1'
  ) {
  ...
  submission.familyDate =
    submission['Family Orientation Attendees'] == 'None'
      ? 'N/A'
      : submission['Family Orientation Date'];
  submission.familyCharge =
    submission['Family Orientation Attendees'] == 'None'
      ? 0
      : parseInt(submission['Family Orientation Attendees']) * 74;
...
function sendEmails(e) {
  ...
  GmailApp.sendEmail(
    ...
    `UMD Orientation Confirmation | Office of Student Orientation & Transition`, // email subject
    htmlBody,
    {
      from: 'askorientation@umd.edu', // send from askorientation
      name: 'UMD Orientation', // name of sender
      htmlBody: htmlBody,
    }
  );
  ...
  if (sheet) {
    ...
    if (
      emailBox &&
      emailBox.getDataValidation().getCriteriaType() ==
        SpreadsheetApp.DataValidationCriteria.CHECKBOX
    ) {
      // Checkboxes were added
